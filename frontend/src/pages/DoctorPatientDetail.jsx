...
const DoctorPatientDetail = () => {
  ...
  const [showDoctorFamilyNote, setShowDoctorFamilyNote] = useState(false);
const [showDoctorStaffNote, setShowDoctorStaffNote] = useState(false);
...
const [doctorData, setDoctorData] = useState(null);
  ...
  const fetchDoctorData = async () => {
    try {
      ...
      if (!snapshot.empty) {
        setDoctorData({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() });
      }
    } catch (error) {
  ...
  const handleSendDoctorNote = async (text) => {
  try {
    const note = {
      id: Date.now().toString(),
      text: text,
    ...
    // Add to patient's familyNotes
    const docRef = doc(db, 'patients', id);
    const docSnap = await getDoc(docRef);
    const data = docSnap.data();
    const updatedNotes = [...(data.familyNotes || []), note];
    ...
    console.error('Error sending doctor note:', error);
    return false;
  }
};
  ...
  const handleSendDoctorFamilyNote = async (e) => {
  e.preventDefault();
  ...
  try {
    ...
    setFormSuccess('Note sent to family successfully!');
    setDoctorNoteText('');
    setTimeout(() => {
      setFormSuccess('');
      setShowDoctorFamilyNote(false);
    }, 3000);
  } catch (error) {
...
const handleSendDoctorStaffNote = async (e) => {
  e.preventDefault();
  if (!doctorNoteText.trim()) {
    setFormError('Please enter a note');
    return;
  ...
  try {
    ...
    setTimeout(() => {
      setFormSuccess('');
      setShowDoctorStaffNote(false);
    }, 3000);
  } catch (error) {
    console.error('Error sending note to staff:', error);
    setFormError('Failed to send note to staff. Please try again.');
  }
};
                          ...
                          <div key={index} className="p-3 bg-white/50 rounded-lg border border-gray-200/50">
                            <div className="grid grid-cols-3 gap-2 text-xs">
                              <div>
                                ...
                                <p className="text-gray-900 font-medium">{v.bp || '--'}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">Pulse</p>
                                <p className="text-gray-900 font-medium">{v.pulse || '--'} bpm</p>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <p className="text-gray-900 font-medium">{v.oxygenSaturation || '--'}%</p>
                              </div>
                              <div>
                                <p className="text-gray-500">RR</p>
                                <p className="text-gray-900 font-medium">{v.respiratoryRate || '--'}/min</p>
                              </div>
                              <div className="col-span-3">
                                <p className="text-xs text-gray-400 mt-1">by {v.recordedBy || 'Staff'} • {new Date(v.recordedAt).toLocaleString()}</p>
                              </div>
                            </div>
                          </div>
                        ))}
  ...
  <div>
    ...
    <div className="space-y-4 max-h-[500px] overflow-y-auto">
      ...
      {/* Doctor to Family Note Modal */}
<AnimatePresence>
  {showDoctorFamilyNote && (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSendDoctorFamilyNote}>
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-white/50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all duration-300 min-h-[120px]"
              rows="4"
              required
            />
          </div>
  ...
  {showDoctorStaffNote && (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSendDoctorStaffNote}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Note *</label>
            <textarea
              value={doctorNoteText}
              ...
              className="w-full px-4 py-3 bg-white/50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 min-h-[120px]"
              rows="4"
              required
            />
          </div>
